...
int RestoreArchive(CODBCQuery& m_query,CDBConn& dbConn,reg::Key& rkMailFlow, TCHAR* wDsn)
{
	SQLRETURN	retSQL = 0;
	...
	if(numFound > 1)
	{
		...
		do
		{
			wsprintf(sLogBuf,_T("Found multiple Archives, enter the ArchiveID you would like to restore:"));
			_tcout << endl << sLogBuf;
			g_logFile.Write(sLogBuf);
			cin >> nArcID;
		}
	...
	retval = GetTicketListFromArchive( m_query, nArcID, msgList, sArchive );
	if(retval == 0)
	{
