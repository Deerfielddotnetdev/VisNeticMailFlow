...
int EncryptMessageSrcPasswords(CDBConn& dbConn,reg::Key& rkMailFlow)
{
	...
	if(g_showResults != 0)
	{
		_tcout << _T("Encrypting MessageSources passwords") << endl;
	}

	g_logFile.Write(_T("Encrypting MessageSources passwords"));
	...
	retSQL = dbConn.GetData(_T("select count(*) from MessageSources"),test);
	...
	retSQL = dbConn.ExecuteSQL(_T("declare i_cursor cursor for select MessageSourceID from MessageSources"));
	if(!SQL_SUCCEEDED(retSQL))
	{
		return ErrorOpt(dbConn,rkMailFlow);
	}
	...
	for(int i = 1; i < num_rows; i++)
	{
		...
		wsprintf(buf,_T("select AuthPassword from MessageSources where MessageSourceID = %s"),test);
		...
		wsprintf(buf,_T("update MessageSources set AuthPassword = '%s' where MessageSourceID = %s"),password,test);
