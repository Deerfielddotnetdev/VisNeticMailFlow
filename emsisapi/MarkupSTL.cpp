...
bool CMarkupSTL::SetDoc( const char* szDoc, size_t nLength )
{
	// Reset indexes
	m_iPosFree = 1;
	ResetPos();
	...
	{
		if ( nLength )
			m_strDoc.assign( szDoc, nLength );
		else
			m_strDoc.assign( szDoc );
	}
	else
	{
		// MJM - if the string has more then 128K allocated, deallocated the memory
		// otherwise just clear the string
		m_strDoc.capacity() > 131072 ? m_strDoc.assign( _T("") ) : m_strDoc.erase();
	}

	// MJM - make sure atleast 64KB is reserved for the string
	m_strDoc.reserve( 65536 );
	
	// Starting size of position array: 1 element per 64 bytes of document
	// Tight fit when parsing small doc, only 0 to 2 reallocs when parsing large doc
	// Start at 8 when creating new document
	UINT nStartSize = m_strDoc.size() / 64 + 8;
...
bool CMarkupSTL::Load( const char* szFileName )
{
	...
	if ( fp )
	{
		...
		// Load string
		std::vector<char> pBuffer(nFileLen + 1);
		if ( fread( (void*)pBuffer.data(), nFileLen, 1, fp ) == 1 )
		{
			pBuffer[nFileLen] = '\0';
			bResult = SetDoc( pBuffer.data() );
		}
		fclose(fp);
	}
	if ( ! bResult )
...
bool CMarkupSTL::AddChildElem(const char* szName, int nValue )
{
	// Convert integer to string and call SetChildAttrib
	char szVal[25];
	sprintf( szVal, "%d", nValue );
	return x_AddElem(szName,szVal,false,true);
}
...
{
	// MJM - let's try to allocate memory a little better....
	if ( ( m_strDoc.capacity() - m_strDoc.size() ) < 1024 )
	{
		m_strDoc.reserve(m_strDoc.capacity() * 2);
	}
